...
installKeploy (){
    ...
    install_keploy_arm() {
        curl --silent -o keployE --location "https://keploy-enterprise.s3.us-west-2.amazonaws.com/releases/latest/enterprise_linux_arm64"

        sudo chmod a+x keployE && sudo mkdir -p /usr/local/bin && sudo mv keployE /usr/local/bin

        set_alias 'sudo -E env PATH="$PATH" keployE'

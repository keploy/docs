...
var TestHelpers = /** @class */ (function () {
     ...
     */
    TestHelpers.parseAndMatchNodeParserSnapshot = function (buffer) {
        var configuration = new TSDocConfiguration_1.TSDocConfiguration();
    ...
    TestHelpers.parseAndMatchDocCommentSnapshot = function (buffer, configuration) {
        ...
        expect({
            ...
            _00_lines: parserContext.lines.map(function (x) { return TestHelpers.getEscaped(x.toString()); }),
            ...
            _06_paramBlocks: docComment.params.blocks.map(function (x) { return TestHelpers.getDocNodeSnapshot(x); }),
            _07_typeParamBlocks: docComment.typeParams.blocks.map(function (x) { return TestHelpers.getDocNodeSnapshot(x); }),
            _08_returnsBlock: TestHelpers.getDocNodeSnapshot(docComment.returnsBlock),
            _09_customBlocks: docComment.customBlocks.map(function (x) { return TestHelpers.getDocNodeSnapshot(x); }),
            _10_inheritDocTag: TestHelpers.getDocNodeSnapshot(docComment.inheritDocTag),
            _11_modifierTags: docComment.modifierTagSet.nodes.map(function (x) { return TestHelpers.getDocNodeSnapshot(x); }),
            _12_logMessages: parserContext.log.messages.map(function (message) { return message.text; })

...
var flatMap = _interopDefault(require('unist-util-flatmap'));
...
var Extractor = /*#__PURE__*/function () {
  function Extractor(_ref) {
    ...
    this.program = ts.createProgram(rootFiles.map(function (file) {
      return path__default.resolve(basedir, file);
  ...
  _proto.getComment = function getComment(token, fileName, overload) {
    ...
    var inputFileName = path__default.resolve(this.basedir, fileName);
    ...
    var customConfiguration = new tsdoc.TSDocConfiguration();
    customConfiguration.addTagDefinition(new tsdoc.TSDocTagDefinition({
    ...
    var tsdocParser = new tsdoc.TSDocParser(customConfiguration);
...
var attacher = function attacher(_ref) {
  ...
  return function transformer(tree) {
    return flatMap(tree, function mapper(parent) {
      ...
      var url$1 = new url.URL(node.url);
      var fileName = url$1.host + url$1.pathname;
      var args = url$1.searchParams;
...
function createCompilerHost(compilerOptions, externalResolutions) {
  ...
  return _extends({}, ts.createCompilerHost(compilerOptions), {
    ...
    fileExists: function fileExists(fileName) {
      ...
      return !!virtualFiles[path.normalize(fileName)] || ts.sys.fileExists(fileName);
    },
    readFile: function readFile(fileName) {
      // console.log('readFile', fileName)
      var virtual = virtualFiles[path.normalize(fileName)];
    ...
    setScriptFileNames: function setScriptFileNames(files) {
      scriptFileNames = files.map(path.normalize); // console.log({ virtualFiles, scriptFileNames })
    },
...
exports.linkDocblocks = attacher;
exports.transpileCodeblocks = attacher$1;
//# sourceMappingURL=remark-typescript-tools.cjs.development.js.map

...
| `--channel-binding-shim`  | from record config       | Enable the eBPF channel-binding shim: on replay it downgrades libpq `channel_binding=require`→`disable` so SCRAM-SHA-256-PLUS apps replay against trust-mode mocks. Needs a Keploy agent image that has cbshim. Mirrors `keploy.yml` `record.channelBindingShim`. |
...
| `sysctl: executable file not found in $PATH`                     | The privileged (sudo) re-exec stripped `/usr/sbin`/`/sbin` from `PATH`                                           | Handled by the CLI automatically; on older versions `export PATH="$PATH:/usr/sbin:/sbin"`     |

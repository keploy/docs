...
Keploy runs natively on macOS (Apple Silicon / arm64) and Windows (x86‑64) in the **[Keploy Community Edition](/docs/server/install/)** and Enterprise builds — which is what `https://keploy.io/install.sh` installs by default. Neither platform has eBPF, so Keploy intercepts traffic in userspace on both, and neither needs `sudo` or Administrator. (The OSS build, `install.sh --oss`, intercepts with eBPF and records a natively-running app on Linux only; on macOS and Windows it works through Docker.) Docker, Lima (macOS) and WSL (Windows) remain supported alternatives, and are still the route on Windows/ARM. On an Intel Mac use Lima: the Docker route on macOS still runs the native CLI on the host, so it is Apple Silicon only too.

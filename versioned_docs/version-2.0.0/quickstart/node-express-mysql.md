...
sidebar_label: NodeJS - Express + mysql
...
A simple sample CRUD application and see how seamlessly Keploy integrates with [Express](https://expressjs.com/) and [MySQL](https://www.mysql.com/). Buckle up, it's gonna be a fun ride! 🎢
...
Clone the repository and move to js-mysql folder

```bash
git clone https://github.com/keploy/samples-typescript && cd samples-typescript/js-mysql
...
- <details>
  ...
  Fire up the application and MySQL instance with Keploy. Keep an eye on the two key flags:
  ...
  curl -X POST http://localhost:3000/items \
     ...
     -d '{"name": "Sample Item", "description": "This is a sample description"
     }'
  ...
  Or simply wander over to your browser and visit `http://localhost:8000/items`.
  ...
  ```bash
  keploy test -c "docker compose up" --containerName "nodesqlapp" --delay 10
  ```
  ...
  ```bash
  sudo -E env PATH=$PATH keploy record -c 'node ./server.js'
  ```
  ...
  ```bash
  sudo -E env PATH=$PATH keploy test -c "node ./server.js" --delay 10
  ```
